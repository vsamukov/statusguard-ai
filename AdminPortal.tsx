...
const AdminPortal: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className="w-20 lg:w-64 bg-indigo-900 text-white flex flex-col transition-all duration-300">
        ...
        <nav className="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
          <p className="hidden lg:block text-[10px] font-black text-indigo-400 uppercase tracking-widest px-4 mb-4">Your Infrastructure</p>
        ...
        <div className="p-4 border-t border-white/10">
          <button 
            onClick={logout}
            className="w-full flex items-center gap-3 p-3 text-red-300 hover:bg-red-500/10 hover:text-red-100 rounded-xl transition-colors"
          >
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="bg-white border-b border-gray-200 h-16 flex items-center px-8 sticky top-0 z-10 shadow-sm">
          <div className="flex justify-between w-full items-center">
             <div className="flex items-center gap-3">
               <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Active Application:</span>
               <span className="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full text-sm font-bold border border-indigo-100">
                 {activeDashboard?.name || 'Connecting...'}
               </span>
               <a 
             ...
             <div className="flex items-center gap-4">
                <span className="text-[10px] font-bold text-gray-400 uppercase">Support Agent: {state.currentUser}</span>
             </div>
          </div>
        </header>
           ...
           {state.activeDashboardId ? (
             ...
             <div className="flex flex-col items-center justify-center h-[60vh] text-center">
                <div className="w-20 h-20 bg-indigo-50 rounded-3xl flex items-center justify-center text-indigo-600 mb-6">
                   <svg className="w-10 h-10 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                </div>
                <h2 className="text-2xl font-bold text-gray-900">Select an application</h2>
                <p className="text-gray-500 mt-2">Use the sidebar to pick a dashboard instance to manage.</p>
             </div>
           )}
        </div>
      </main>

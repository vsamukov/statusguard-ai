...
export const nodeApi = {
  ...
  async createSubscriber(email: string, regionIds: string[]) {
    const url = `/api/subscriptions`;
    const res = await fetch(url, { 
      method: 'POST', 
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, regionIds }) 
    });
    return handleResponse(res, url);
  },
...
export const createRemoteApi = (config: RemoteDashboardConfig, token?: string | null) => {
  ...
  return {
    ...
    async createSubscriber(email: string, regionIds: string[]) {
      ...
      const res = await safeFetch(url, { method: 'POST', headers, body: JSON.stringify({ email, regionIds }) });
      return handleResponse(res, url);
    },
